		...
		<div className="w-fit inline-flex gap-3">
			{typedKeys(filterOptions).map((filter) => (
				<button
					key={filter}
					type="button"
					className={`py-2.5 px-5 mb-2 text-sm font-bold focus:outline-none rounded-2xl text-grey-content  ${filterBy === filter ? 'nm-inset-medium' : 'nm-flat-medium hover:nm-flat-soft'}`}
