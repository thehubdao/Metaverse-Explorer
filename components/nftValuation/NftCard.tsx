...
interface INftCard {
    ...
    tokenId: string
    listedPrice: number | undefined | null
    name: string | null
}
...
const NftCard = ({ image, predictedPrice, listedPrice, collectionName, tokenId, name }: INftCard) => {
    ...
    return (
        <div className={`grid grid-rows-3 rounded-xl cursor-pointer lg:w-[200px] lg:h-[300px] 2xl:w-[240px] 2xl:h-[360px] focus:outline-none nm-flat-hard  hover:nm-flat-soft transition duration-300 ease-in-out`}>
            ...
            <div className="flex flex-col justify-around py-1">
                <p className="text-center font-bold">{name ? name : `${collectionName.toUpperCase()} #${tokenId}`}</p>
                <div>
                    <div className="flex w-full flex-row">
                        {listedPrice
                            ? <BoxPrice text='Listed Price' price={formatter.format(listedPrice)} />
                            : <NoListed text='Listed Price' />
                        }
                        <BoxPrice text='Price Estimation' price={formatter.format(predictedPrice)} />
                    </div>
                    {listedPrice && (listedPrice > predictedPrice && <p className='text-red-500 w-full text-center text-xs pt-1'>Overvalue</p>)}
                </div>
            </div>
        </div>
    )
