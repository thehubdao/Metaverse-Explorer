...
export async function addLandToWatchList(
    ...
    const addToWatchListRequest = await axios.post(`${process.env.AUTH_SERVICE}/watchlistService/addToWatchlist?address=${address}&metaverse=${metaverse}`, land, {
        headers: {
...
export async function removeLandFromWatchList(
    ...
    const removeFromWatchListRequest = await axios.post(`${process.env.AUTH_SERVICE}/watchlistService/removeFromWatchList?address=${address}&metaverse=${metaverse}`, land, {
        headers: {
            'Content-Type': 'application/json',
            'Authentication': `${token}`
        }
...
export async function updateCallsCount(address:any, callsCountAdd:number,token: string){
    const updateCallsCount = await axios.post(`${process.env.AUTH_SERVICE}/watchlistService/updateCallsCount?address=${address}&callsCountAdd=${callsCountAdd}`,{ }, {
        headers: {
            'Content-Type': 'application/json',
            'Authentication': `${token}`
        }
...
export async function getCallsCount(address:any, token: string){
    const getCallsCount = await axios.get(`${process.env.AUTH_SERVICE}/watchlistService/getCallsCount?address=${address}`, {
        headers: {
            'Content-Type': 'application/json',
            'Authentication': `${token}`
        }
